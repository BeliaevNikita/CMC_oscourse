...
nvme_map(struct NvmeController *ctl) {
    ...
    // MYTODO LAB 10: Add error checking + reorder min + 80 symbols ber line
    uintptr_t nvme_pa = get_bar_address(ctl->pcidev, 0);
    size_t memsize = MIN(NVME_MAX_MAP_MEM, get_bar_size(ctl->pcidev, 0));
    
    if (sys_map_physical_region(nvme_pa, CURENVID, (void *) ctl->mmio_base_addr, memsize, PROT_RW | PROT_CD) != 0) {
        return -NVME_MAP_ERR;
    }
